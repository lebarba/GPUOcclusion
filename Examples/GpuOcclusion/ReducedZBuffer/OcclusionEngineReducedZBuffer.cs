...
{
    /// <summary>
    /// Tecnica con 1 solo Z Buffer de tamaño reducido
    /// </summary>
    public class OcclusionEngineReducedZBuffer
    {
        ...
        public void init(int maxOccludees)
        {
            ...
            occlusionViewport.Width = screenViewport.Width / viewportReduced;
            occlusionViewport.Height = screenViewport.Height / viewportReduced;


            //Create the Occlusion map
            //hiZBufferWidth = GpuOcclusionUtils.getNextHighestPowerOfTwo(occlusionViewport.Width);
            //hiZBufferHeight = GpuOcclusionUtils.getNextHighestPowerOfTwo(occlusionViewport.Height);
            hiZBufferWidth = occlusionViewport.Width;
            hiZBufferHeight = occlusionViewport.Height;
            hiZBufferTex = new Texture(d3dDevice, hiZBufferWidth, hiZBufferHeight, 1, Usage.RenderTarget, Format.R32F, Pool.Default);
            ...
            occlusionEffect = ShaderUtils.loadEffect(GuiController.Instance.ExamplesMediaDir + "Shaders\\ReducedZBuffer\\OcclusionEngineReducedZBuffer.fx");
        ...
        private void updateOccludeesData()
        {
            ...
            for (int i = 0; i < enabledOccludees.Count; i++)
            {
                ...
                if (GpuOcclusionUtils.projectBoundingBox(enabledOccludees[i].BoundingBox, occlusionViewport, out meshBox2D))
                {
