...
Unlike the HOMís Occlusion Map \cite{hom}, our map does not contain opacity information, therefore the buffer is more similar 
...
\begin{figure}[!hbp]
	\oneimage{First step is to obtain the simplified occluders as proxy meshes. Second step is to render all proxy meshes to the \emph{Occlusion Map} texture.}{.99}{occluder_generation.png}
\end{figure}
...
\begin{figure}[!hbp]
	\oneimage{The occludees in the scene are projected in 2D and the Bounding Rectangle is calculated. 
	...
	{.99}{visibility_test.png}
	\label{img_visibility_test}
\end{figure}
...
orienting it  to the work presented by Nieﬂner et al \cite{occlusion_culling_tessellation} and [CITAR R·kos]. 
