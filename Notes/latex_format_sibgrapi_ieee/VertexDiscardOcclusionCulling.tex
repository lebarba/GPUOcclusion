...
These simplified occluders will be rendered faster than the original meshes at the expense of more conservativeness \figref{occluder_generation}.\
...
The resulting occludee bounding rectangle becomes a conservative superset of the actual pixels covered by the occludee  (see \figref{visibility_test}). 
...
{The occludees in the scene are projected in 2D and the Bounding Rectangle is calculated. 
	...
	storing the visibility result in the \emph{Visibility Map}.}
{visibility_test}{
...
the block visibility results one next to the other as seen in \figref{visibility_blocks}. 
...
\subimages[!hbp]
{The occludee is split into 8x8 blocks, then each block performs the visibility test and stores the result into the \emph{Unreduced Visibility Map}. Each occludee has an pre-assigned region of 32x32 blocks inside this Map.  \emph{Occlusion Map} texture.}
{visibility_blocks}{
\subimage[]{}{visibility_blocks.png}%
}
...
We tested our method in a densely occluded 3D city scene \figref{city_images}, composed of 210 meshes, adding up a total of 379.664 triangles. 
...
\subimages[!hbp]
{Top: The 3D city scene used to test the algorithm. Bottom: The simplified occluder set used for Occlusion Culling}
{city_images}{
\subimage[]{}{city_images.png}%
}
...
With this metric we can determine the percentage of meshes that were discarded by the GPU in each frame due to occlusion culling (see results in \figref{discarded_objects_percent}).\

These values are computed with Occlusion Culling deactivated and then with it activated. 
We also include the frames per second that resulted from rendering the scene with and without Occlusion Culling (see results in \figref{discarded_objects_fps}). 
The values were computed using a PC with Intel Core i3 2.40GHz processor with 2GB RAM and Intel HD Graphics 3000 GPU.


\subimages[!hbp]
{Discarded mesh percent, first with only Frustum Culling and then activating Occlusion Culling, at the fifteen different selected view points.}
{discarded_objects_percent}{
\subimage[]{}{discarded_objects_percent.png}%
}


\subimages[!hbp]
{FPS rendering performance only with Frustum Culling and then with Occlusion Culling activated, at the 15 different selected view points.}
{discarded_objects_fps}{
\subimage[]{}{discarded_objects_fps.png}%
}
...
\begin{figure}[!hbp]
	\oneimage{Top: The original city scene without Occlusion Culling. Middle: The scene with Occlusion Culling. Bottom: The yellow boxes show the occluded meshes of the scene.}
	{.99}{final_images.png}
\end{figure}
